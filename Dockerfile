...
#
FROM            nginx:alpine
MAINTAINER      Leviscop <git@leviscop.dev>
...
RUN echo '#!/bin/sh'                                                                                         > /entrypoint/primer.sh && \
	...
	echo 'echo "map \$http_upgrade \$connection_upgrade {"                                   >> default.conf' >> /entrypoint/primer.sh && \
	echo 'echo "    default upgrade;"                                                        >> default.conf' >> /entrypoint/primer.sh && \
	echo 'echo "    \"\"      close;"                                                        >> default.conf' >> /entrypoint/primer.sh && \
	echo 'echo "}"                                                                           >> default.conf' >> /entrypoint/primer.sh && \
	echo 'echo ""                                                                            >> default.conf' >> /entrypoint/primer.sh && \
